    ...
    function filterResultsOnCompleted() {
        ...
        if (!isFilterByCompleted) {
            clonedArray
                ...
                .map((task, index) => (task.hidden = true))

            setTasks(clonedArray)
        } else {
            clonedArray
                .filter((task) => task.hidden)
                .map((task, index) => (task.hidden = false))
        ...
        <div className="container">
            <div className="Sidebar">
                ...
                <div className="sidebar-functionality">
                    ...
                    <button
                        className="sidebar-button"
                        onClick={handleSortAlphabeticallyReversed}
                    >
                        Sort alphabetically (z to a)
                    </button>
                    <button
                        className="sidebar-button"
                        onClick={handleSortByDateCreatedEarliest}
                    >
                        Sort by date created (earliest to latest){' '}
                    </button>
                    <button
                        className="sidebar-button"
                        onClick={handleSortByDateCreatedLatest}
                    >
                        Sort by date created (latest to earliest){' '}
                    </button>
                    <p>
                        <Checkbox
                            id="1"
                            style={{ color: 'red' }}
                            checked={isFilterByCompleted}
                            defaultChecked={false}
                            disabled={isFilterByPending}
                            onChange={filterResultsOnCompleted}
                        ></Checkbox>
                        Filter by tasks completed{' '}
                        {isFilterByPending && <div>disabled</div>}
                    </p>
                    <p>
                        <Checkbox
                            id="2"
                            style={{ color: 'red' }}
                            checked={isFilterByPending}
                            defaultChecked={false}
                            disabled={isFilterByCompleted}
                            onChange={filterResultsOnPending}
                        ></Checkbox>
                        Filter by tasks pending{' '}
                        {isFilterByCompleted && <div>disabled</div>}
                    </p>
                    <p>
                        filter by tag
